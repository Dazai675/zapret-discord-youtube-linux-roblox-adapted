...
start "zapret: %~n0" /min "%BIN%winws.exe" --wf-tcp=80,443,1024-65535,%GameFilter% --wf-udp=443,50000-50100,1024-65535,%GameFilter% ^
...
--filter-tcp=1024-65535 --ipset="%LISTS%ipset-all.txt" --dpi-desync=fake,split2 --dpi-desync-repeats=8 --dpi-desync-fooling=md5sig --dpi-desync-autottl=2 --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --new ^
--filter-udp=1024-65535 --ipset="%LISTS%ipset-all.txt" --dpi-desync=fake --dpi-desync-autottl=2 --dpi-desync-repeats=10 --dpi-desync-any-protocol=1 --dpi-desync-fake-unknown-udp="%BIN%quic_initial_www_google_com.bin" --dpi-desync-cutoff=n2
